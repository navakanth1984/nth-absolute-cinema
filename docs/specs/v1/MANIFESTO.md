...
3. **Humans direct; AI proposes.** Every compiler's output is a proposal subject to
   the review workflow (`CREATIVE_GRAPH_SPEC.md` §7 Review Graph node types); no artifact reaches
...
4. **Outputs must be reproducible through provenance.** Every artifact carries enough
   metadata (`COMPILER_ABI.md` §Provenance contract) to regenerate it exactly, or to explain
...
6. **Quality and consistency outweigh generation speed.** Compilers are permitted to
   retry, validate, and repair (`COMPILER_ABI.md` §Compiler contract (the 8-stage pipeline)) rather than
...
7. **Economic efficiency is a first-class optimization objective.** Every compiler
   ...
   §Estimation contract) — this is not optional instrumentation, it is a contract requirement.
